%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% CCAF_ABM_BF: Adaptive Blocking Matrix with Coefficient Constrained Adatpive Filter
...
%	
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function beamformingout=CCAF_ABM_BF(mic_array_input)
global Cfg;
...
ABM_reg=zeros(Cfg.SimMicNum,CCAF_FiltL);
ABM_W=zeros(Cfg.SimMicNum,CCAF_FiltL);
ABM_W_pre=zeros(Cfg.SimMicNum,CCAF_FiltL);
...
%}
ANC_reg=zeros(Cfg.SimMicNum,ANC_FiltL);
ANC_W=zeros(Cfg.SimMicNum,ANC_FiltL);
M=-2:2;
%{
...
%X'(t)
dly_steer_out=zeros(Cfg.SimMicNum,src_len);
for i=1:Cfg.SimMicNum
	dly_steer_out(i,1+CCAF_delay:end)=mic_steer_out(i,1:end-CCAF_delay);
end

beamformingout=zeros(1,src_len);
z=zeros(1,src_len);
yA=zeros(Cfg.SimMicNum,src_len);
reb_y=zeros(Cfg.SimMicNum,src_len);
...
for i=1:sim_len
	...
	for mic_idx=1:Cfg.SimMicNum
		ABM_reg(mic_idx,:)=[FBFout(i),ABM_reg(mic_idx,1:end-1)];
		if Cfg.GenieEn==1
			%{
	...
	%ANC input phase
	for mic_idx=1:Cfg.SimMicNum
		ANC_reg(mic_idx,:)=[yA(mic_idx,i),ANC_reg(mic_idx,1:end-1)];
	end

	%ANC output phase
	c(i)=sum(sum(ANC_W.*ANC_reg));
	...
	if flag
		for mic_idx=1:Cfg.SimMicNum
			ABM_W(mic_idx,:)=ABM_W(mic_idx,:)+ccaf_u*yA(mic_idx,i)*ABM_reg(mic_idx,:);
		end
		if Cfg.CCAF_MaskEn
			for mic_idx=1:Cfg.SimMicNum
				for k=1:CCAF_FiltL
	...
	ABM_MMSE(i)=sum(sum((ABM_W-ABM_W_pre).^2));
	%{
...
if Cfg.DebugEn && bitand(Cfg.DebugMask,hex2dec('10'))
	...
	hold on;plot(dly_steer_out(1,:),'m');
	hold on;plot(reb_y(1,:),'g');
	%{
	tmp=filter(ABM_W(1,:),1,FBFout);
	hold on;plot(tmp,'gO-');
